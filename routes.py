...
def updatediagnostics():
    if 'username' in session:
        usern = session['username']
        print(usern)
        updatet = DiagnosticsMaster.query.all()
        print(updatet)
        if not updatet:
            flash('No Tests exists in database')
            return redirect( url_for('addDiagnostics') )
        else:
            print("inside else")
            return render_template('updatediagnostics.html', updatet = updatet)

    else:
        flash('You have been logged out. Please login again')
        return redirect( url_for('login') )

@app.route('/editdiagnosticsdetail/<tid>', methods=['GET', 'POST'] )
def editdiagnosticsdetail(tid):
    print("id is : ", tid)
    if 'username' in session:
        print("inside sess")
        print(datetime.now())
        editpat = DiagnosticsMaster.query.filter_by( tid = tid )
        

        if request.method == 'POST':  
            print("inside editpat post ptd")
            tname = request.form['tname']      
            tcharge = request.form['tcharge']
            row_update = DiagnosticsMaster.query.filter_by( tid = tid ).update(dict(mname=tname,  tcharge=tcharge))
            db.session.commit()
            print("Roww update", row_update)
            ...
            if row_update == None:
                ...
                flash('Test update initiated successfully')
                return redirect( url_for('diagnosticsstatus') )

        return render_template('editdiagnosticsdetail.html', editpat = editpat)

@app.route('/deletediagnostics')
def deletediagnostics():
    if 'username' in session:
        ...
        if not updatet:
            ...
            print("inside else")
            return render_template('deletediagnostics.html', updatet = updatet)

    else:
        flash('You have been logged out. Please login again')
        return redirect( url_for('login') )
    
@app.route('/deletediagnosticsdetail/<tid>')
def deletediagnosticsdetail(tid):
    if 'username' in session:
        ...
        if delpat == None:
            ...
            flash('Test deletion initiated successfully')
            return redirect( url_for('diagnosticsstatus') )
